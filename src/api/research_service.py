...
class ResearchService:
    ...
    async def execute_query(self,query: str,num_results:int = 5) -> Dict[str,Any]:
        if not self.initialized:
            raise RuntimeError("Research service not initialised")
    ...
    async def execute_query_streaming(
        ...
        if not self.initialized:
            raise RuntimeError("Resaerch service not initialized")
        ...
        try:
            ...
            yield StreamChunk(
                type="analysis_start",
                content="Analyzing findings and generatong insights...",
            ...
            yield StreamChunk(
                type="error",
                content=f"Error durng reasarch: {str(e)}",
